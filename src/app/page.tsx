...
export type StatusFilter = 'all' | 'complete' | 'requested' | 'not-indexed';
...
export default function Home() {
  ...
  const [statusMap, setStatusMap] = useState<Record<string, 'complete' | 'requested' | 'not-indexed'>>({});

  type IndexStatus = 'complete' | 'requested' | 'not-indexed';

  const normalizeStatus = (val: string | undefined | null): IndexStatus => {
    const s = (val || '').toString().toLowerCase();
    if (s.includes('complete') || s === 'indexed' || s === 'indexing complete') return 'complete';
    if (s.includes('request')) return 'requested';
    if (s === 'not-indexed' || s.includes('not')) return 'not-indexed';
    return 'requested';
  };

  // Load indexing statuses generated by a scheduled script into /public/indexing-status.json
  useEffect(() => {
    const loadStatuses = async () => {
      try {
        const res = await fetch('/indexing-status.json', { cache: 'no-store' });
        if (!res.ok) return;
        const data = await res.json();
        const map: Record<string, IndexStatus> = {};
        if (Array.isArray(data)) {
          for (const item of data) {
            ...
            if (slug) map[slug] = normalizeStatus(item.status);
          }
        } else if (data && typeof data === 'object') {
          for (const [slug, status] of Object.entries<any>(data)) { // eslint-disable-line @typescript-eslint/no-explicit-any
            map[slug] = normalizeStatus(status as string);
          }
        }
        setStatusMap(map);
      } catch {
        // Ignore; demo will fall back to heuristic below
      }
    };
    loadStatuses();
  }, []);

  const getHeuristicStatus = (item: ReflectionData): IndexStatus => {
    const fromMap = statusMap[item.slug as string];
    if (fromMap) return fromMap;
    const now = Date.now();
    const published = new Date(item.publishDate).getTime();
    const days = (now - published) / (1000 * 60 * 60 * 24);
    return days <= 3 ? 'requested' : 'not-indexed';
  };
  ...
  useEffect(() => {
    const filtered = sortedData.filter(item => {
      ...
      const status = getHeuristicStatus(item);
  ...
  }, [searchTerm, startDate, endDate, statusFilter, statusMap, sortedData]);
          ...
          {filteredData.length === 0 ? (
            ...
            filteredData.map((reflection) => (
              <ReflectionCard 
                ...
                status={getHeuristicStatus(reflection)}
              />
            ))
          )}
        </div>
